...
func(this*UserController)HandleReg(){
	...
	//发送邮件
	emailConfig := `{"username":"1554190560@qq.com","password":"hdulrbptfhnuidfc","host":"smtp.qq.com","port":587}`
	emailConn := utils.NewEMail(emailConfig)
	emailConn.From = "1554190560@qq.com"
	emailConn.To = []string{email}
	emailConn.Subject = "天天生鲜用户注册"
	//注意这里我们发送给用户的是激活请求地址
	emailConn.Text = "192.168.150.20:8080/active?id="+strconv.Itoa(user.Id)
...
func (this*UserController)ShowUserCenterInfo()  {
	...
	//获取历史浏览记录
	conn,err := redis.Dial("tcp","192.168.150.20:6379")
	defer conn.Close()
	if err != nil {
		beego.Info("链接错误")
	}
